			...
			match segment {
				...
				CssClassNameSegment::Name => {
					match &*self.file_name {
						...
						_ => {
							unreachable!(
								"CssModuleTransformConfig::new_name_for: \
								 invalid file name"
							)
						},
					}
			...
			if s.starts_with('[') {
				if let Some(end_idx) = s.find(']') {
					let segment = match &s[0..=end_idx] {
						...
						s => {
							bail!(
								"Unknown placeholder {} at {} in CSS Modules \
								 pattern: {}",
								s,
								idx,
								self.pattern
			...
			let code = {
				let mut buf = String::new();
				{
					let wr = BasicCssWriter::new(
						&mut buf,
						if opts.source_map { Some(&mut src_map) } else { None },
						BasicCssWriterConfig {
			...
			let map = if opts.source_map {
				...
				Some(
					String::from_utf8(buf)
						.context("the generated source map is not utf8")?,
				)
			} else {
				None
			};
			...
			if let Some(config) = opts.css_modules {
				let result = swc_css_modules::compile(
					...
					CssModuleTransformConfig {
						...
						pattern:config.parse_pattern().context(
							"failed to parse the pattern for CSS Modules",
						)?,
					},
				);
				let map:HashMap<_, _> = result
					...
					.map(|(k, v)| {
						(
							...
							v.into_iter()
								.map(|v| {
									match v {
									swc_css_modules::CssClassName::Local {
										name,
									} => CssClassName::Local { name:name.value },
									swc_css_modules::CssClassName::Global {
										name,
									} => {
										CssClassName::Global { name:name.value }
									},
									swc_css_modules::CssClassName::Import {
										name,
										from,
									} => {
										CssClassName::Import {
					...
					.collect();
				Some(serde_json::to_string(&map).context(
					"failed to serialize the mapping for CSS Modules",
				)?)
			} else {
				None
			};
		...
		let code = {
			let mut buf = String::new();
			{
				...
				let mut gen = CodeGenerator::new(
					wr,
					CodegenConfig { minify:opts.minify },
				);
